...
class FavouriteAdapter(private val context: Context, private var musicList: MutableList<Music>,val playNext: Boolean = false) : RecyclerView.Adapter<FavouriteAdapter.MyHolder>() {
    ...
    override fun onBindViewHolder(holder: MyHolder, position: Int) {
        ...
        if(playNext){
            ...
            holder.root.setOnLongClickListener {
                ...
                bindingMF.AddToPNBtn.text = "Loại bỏ"
                bindingMF.AddToPNBtn.setOnClickListener {
                    if(position == PlayerActivity.songPosition)
                        Snackbar.make((context as Activity).findViewById(R.id.linearLayoutPN),
                            "Không thể loại bỏ bài hát đang phát", Snackbar.LENGTH_SHORT).show()
                    else{
    ...
    fun updateFavourites(newList: MutableList<Music>){
